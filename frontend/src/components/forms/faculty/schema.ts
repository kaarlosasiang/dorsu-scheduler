...
export const facultyBaseSchema = z.object({
  ...
  program: z.string().min(2, "Program must be a valid selection"),
  ...
  adminLoad: z.number().min(0, "Admin load cannot be negative").default(0),
...
type FacultyBaseInput = z.infer<typeof facultyBaseSchema>;

function applySharedFacultyRefinements<T extends z.ZodType<FacultyBaseInput>>(schema: T) {
  return schema
...
export const facultyCreateSchema = applySharedFacultyRefinements(
  facultyBaseSchema.extend({
...
export const facultyEditSchema = applySharedFacultyRefinements(facultyBaseSchema);

/** @deprecated Use facultyCreateSchema or facultyEditSchema by form mode */
export const facultySchema = facultyCreateSchema;
